<template>
  ...
  <div class="q-pa-md">
    ...
    >
      <template #body="props">
        <q-tr :props="props" :class="props.rowIndex === 0 ? 'bg-green-1 text-bold' : ''">
          <q-td v-for="col in props.cols" :key="col.name" :props="props">
            <template v-if="props.rowIndex === 0 && col.name === 'korisnik'">
              <q-icon name="emoji_events" color="green" class="q-mr-xs" />{{ col.value }}
            </template>
...
export default {
  ...
  mounted() {
    ...
    this.socket.on("cijena_azurirana", (data) => {
      if (data.id_predmeta != this.id_predmeta) return;
      this.item.trenutna_cijena = data.nova_cijena;
      axios.get(baseUrl + "get-ponuda/" + this.id_predmeta).then((res) => {
        this.ponude = res.data;
      });
      this.prices = [
        { label: "+ 10%: " + (data.nova_cijena * 1.1).toFixed(2) + " $", value: (data.nova_cijena * 1.1).toFixed(2) },
        { label: "+ 20%: " + (data.nova_cijena * 1.2).toFixed(2) + " $", value: (data.nova_cijena * 1.2).toFixed(2) },
        { label: "+ 30%: " + (data.nova_cijena * 1.3).toFixed(2) + " $", value: (data.nova_cijena * 1.3).toFixed(2) },
        { label: "+ 40%: " + (data.nova_cijena * 1.4).toFixed(2) + " $", value: (data.nova_cijena * 1.4).toFixed(2) },
        { label: "+ 50%: " + (data.nova_cijena * 1.5).toFixed(2) + " $", value: (data.nova_cijena * 1.5).toFixed(2) },
        { label: "+ 100%: " + (data.nova_cijena * 2).toFixed(2) + " $", value: (data.nova_cijena * 2).toFixed(2) },
      ];
    });
