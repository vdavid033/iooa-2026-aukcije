...
app.post('/unostrenutnaponuda', authJwt.verifyTokenUser, function (request, response) {
  const data = request.body
  const ponuda = [
    [data.vrijednost_ponude, data.vrijeme_ponude, data.id_korisnika, data.id_predmeta],
  ]
  connection.query(
    ...
    connection.query(
      ...
      function (error, results, fields) {
        if (error) {
          ...
          return response
            .status(500)
            .json({ error: true, message: 'Registracija korisnika neuspješna.' })
        }
...
app.put('/api/brisanjekorisnika/:idKorisnika', authJwt.verifyTokenAdmin, (req, res) => {
  ...
  connection.query(
    'UPDATE korisnik SET ime_korisnika = ?, prezime_korisnika = ?, email_korisnika = ?, lozinka_korisnika = ?, adresa_korisnika = ? WHERE id_korisnika = ?',
    ['obrisani', 'korisnik', rnd_lozinka, rnd_lozinka, 'obrisani korisnik', req.params.idKorisnika],
    (error, results) => {
      if (error) throw error
      res.send(results)
    }
